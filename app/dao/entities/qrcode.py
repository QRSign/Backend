...
def qrcodes_get_method(session):
    ...
    if not qrcodes:
        ...
        return jsonify([x.serialize(True) for x in qrcodes]), 200
...
def qrcode_get_method(session, id):
    ...
    return jsonify(qrcode.serialize(True)), 200
...
def qrcode_get_method_by_token(session, token):
    ...
    if not signatures:
        ...
        return jsonify(signatures[0].serialize(True)), 200
...
def qrcode_post_method(json, session):
    ...
    return jsonify(qrcode.serialize(True)), 201
...
def qrcode_delete_method(session, id):
    ...
    return jsonify(qrcode.serialize(False)), 200
...
class Qrcode(Entity, Base):
    ...
    created_by = relationship("CathoUser", lazy='subquery')
    ...
    def serialize(self, serialize_user):
        return {
            'id': self.id,
            'title': self.title,
            'token': self.token,
            'user': self.created_by.serialize if serialize_user else self.created_by_id,
